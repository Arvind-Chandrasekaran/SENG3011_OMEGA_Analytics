...
def analyze():
    ...
    try:
        ...
        if (response.json()['cleaned_data'][0]['events'][0]['time_object']['timestamp'] is not None) and (response.json()['cleaned_data'][0]['events'][0]['time_object']['duration_unit'] is not None):
